...
export class SettingService {
  ...
  readonly allShipping = (): Observable<ShipSettingMapper[]> =>
    this.http
      .get<
        ShipSettingMapper[]
      >(`${this.HOST}api/v1/shipping`, { responseType: 'json', withCredentials: true })
      .pipe(
        tap((arr): void => {
          this.tempShipSetting = arr;
        }),
      );

  create = (obj: ShippingDTO): Observable<number> =>
    this.http
      .post<ShippingDTO>(`${this.HOST}api/v1/shipping`, obj, {
        observe: 'response',
  ...
  update = (obj: ShipSettingMapper): Observable<number> =>
    this.http
      .put<ShipSettingMapper>(`${this.HOST}api/v1/shipping`, obj, {
        observe: 'response',
        withCredentials: true,
      })
      .pipe(
  ...
  delete = (shippingId: number): Observable<number> =>
    this.http
      .delete<any>(`${this.HOST}api/v1/shipping/${shippingId}`, {
        observe: 'response',
        withCredentials: true,
      })
      .pipe(
  ...
  readonly allTaxSetting = (): Observable<TaxSetting[]> =>
    this.http
      .get<
        TaxSetting[]
      >(`${this.HOST}api/v1/tax`, { responseType: 'json', withCredentials: true })
      .pipe(
        tap((arr): void => {
          this.tempTaxSetting = arr;
        }),
      );

  updateTaxSetting = (obj: TaxSetting): Observable<number> =>
    this.http
      .put<TaxSetting>(`${this.HOST}api/v1/tax`, obj, {
        observe: 'response',
        withCredentials: true,
      })
      .pipe(
