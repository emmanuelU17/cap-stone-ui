  ...
  template: `
    <div class="p-3">
      <div class="w-full flex justify-center">
        <h1 class="w-fit text-xs border-b border-[var(--app-theme)]">New Shipping Setting</h1>
      </div>

      <form class="w-full p-2 grid grid-cols-1 gap-2 bg-[var(--white)]" [formGroup]="form">
        <!-- Country -->
        <div class="p-4 text-left rounded-md border border-[var(--active)] border-solid bg-[var(--white)]">
...
export class CreateShippingComponent {
  ...
  create(): Observable<number> {
    ...
    return (!country || !usd || !ngn)
      ? of(0)
      : this.service.create({ country: country, usd_price: usd, ngn_price: ngn })
        .pipe(
          tap(() => this.cancel()),
