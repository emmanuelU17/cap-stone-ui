...
export class ProductComponent {
  productDetails$: Observable<State<ProductDetail[]>>;
  ...
  reactiveForm = new FormGroup({
    ...
    qty: new FormControl('quantity', [Validators.required]),
  });
  ...
  constructor(private productService: ProductService, private route: ActivatedRoute) {
    ...
    const uuid: string = id === null ? '' : id;

    // Fetch ProductDetail on load of application
    this.productDetails$ = this.productService.fetchProductDetails(uuid).pipe(
  ...
  onclickColour(event: Event): void {
    ...
    this.currentItem$ = this.productDetails$.pipe(
      map((arr: State<ProductDetail[]>) => {
        ...
        const detail = obj.data.data.find((detail: ProductDetail) => detail.sku === sku);
